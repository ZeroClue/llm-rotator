...
- **Persona model (ADR 0002, ROADMAP #19)**: every node presents one consistent client persona — User-Agent + fingerprint label derived from a stable hash of `node_id` over curated API-client stacks (`PERSONA_N_USER_AGENT`/`PERSONA_N_FINGERPRINT` override). The persona UA always replaces the client's UA upstream. Don't add per-attribute rotation or browser-stack personas: attribute mixing and browser-on-API fingerprints are exactly what makes rotated nodes linkable.
- **Transport flag & fingerprints (issue #47)**: `TRANSPORT=curl_cffi` (optional dep; startup fails loudly if missing or if a curated stack label would be an invalid impersonate target — never silently degrade) swaps the shared requests session for one curl_cffi session per persona fingerprint, cookies cleared post-exchange (per-persona sessions already make cross-persona leakage impossible; clearing is jar hygiene). The adapter translates curl_cffi exceptions into the requests hierarchy and serves case-insensitive headers — send()'s classification and parse_quota_headers depend on both. `resolve_impersonation` maps labels: bare names = impersonate targets, `ja3:`/`akamai:` prefixes = raw specs; curated stack LABELS are intentionally not impersonation targets. Under default `requests`, only the persona UA is observable — documented degradation, don't fake it.
- **Docker image manifest is hand-maintained**: the Dockerfile COPYs named files (`rotator.py failover.py telemetry.py admin_dashboard.py gunicorn.conf.py` + `templates/` + `static/`), not the whole directory. `create_app()` imports the dashboard modules at startup, so a new top-level module missing from the COPY kills the container instantly while local pytest stays green — CI's smoke test is the only catcher (bit us once, PRs #65-#68 merged red). Adding a module? Update the Dockerfile in the same PR, and check CI before merging.
- **Three manifests fail silently when files are added** — audit all of them in the same PR as any new file or directory:
  1. **Dockerfile** COPY whitelist (above): file absent from the image.
  2. **`.dockerignore`**: a matching pattern silently drops the file from the build context — the COPY can't save you.
  3. **`.gitignore`**: `git add -A` silently skips matching paths — the file never gets committed, so *other* checkouts (and CI) build without it even if yours works.
  All three leave local tests green; only CI's container smoke test catches the last one, and none catch the first two.
- **Views must never `import rotator`**: under `python rotator.py` the running app is module `__main__`, so a by-name import re-executes rotator as a second, empty copy (all globals None) — 500s that only appear as a subprocess, never in-process. Cross-module state goes through closures/providers injected in `create_app()` (see `_admin_context`).
- **curl_cffi ignores `chunk_size`** (warns "there is no way to tell curl that"): its `iter_content()` yields network-read granularity natively — per-SSE-event, better latency than requests' byte reads. The response shim deliberately does not forward chunk_size; don't "fix" that back.
- **Telemetry is suite-global state**: `rotator.telemetry` accumulates ring/buckets across tests, so tests asserting exact counts must monkeypatch a fresh `Telemetry()` (see `fresh_telemetry` fixture in tests/test_admin_dashboard.py). Also: `ring_snapshot()` returns defensive copies — `complete_request()` needs the original reference from `record_request()`.
- **Failover ladder & replay hygiene (issue #46)**: `ANONYMITY_FAILOVER=same` waits out short 429 `Retry-After`s on the failing persona (bounded by `FAILOVER_MAX_WAIT`, gate-capped by `FAILOVER_MAX_WAITERS`) and spends the request's single redistribute budget on its first cross-persona hop — cap-hit or too-long `Retry-After` both consume it; afterwards normal selector rotation applies. Default `cross` is availability-first. JSON bodies are re-serialized per attempt (`REDISTRIBUTION_JITTER`, deterministic variants in `serialize_attempt`) so no two *consecutive* attempts carry identical bytes — don't "simplify" back to sending the caller's original bytes on every attempt, and don't let the ladder engage for timeout/5xx (those may have completed upstream; see RETRY_POSTS).
...
- **Budget-aware routing (issue #48)**: successful responses' OpenAI-style ratelimit headers (`x-ratelimit-*`) feed per-node headroom into the ledger; `NodeSelector` prefers healthy > low/unknown > depleted, first-in-cursor within class. All-unreported pools select exactly like the old round-robin; reset windows refill deterministically off the transport's injected clock. Don't rank unreported nodes below reported ones — unknown providers keep their fair share.
...
Before committing anything, opening a PR, or merging: follow the gated implementation workflow (`edukai/`-only commits are exempt via its fast lane). See `docs/agents/implementation-workflow.md`.
