...
- **Transport flag & fingerprints (issue #47)**: `TRANSPORT=curl_cffi` (optional dep; startup fails loudly if missing — never silently degrade) swaps the shared requests session for one curl_cffi session per persona fingerprint, cookies cleared post-exchange (no policy hook exists there). `resolve_impersonation` maps labels: bare names = impersonate targets, `ja3:`/`akamai:` prefixes = raw specs; curated stack LABELS are intentionally not impersonation targets. Under default `requests`, only the persona UA is observable — documented degradation, don't fake it.
